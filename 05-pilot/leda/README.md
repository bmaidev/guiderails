...
| File | What it is |
|---|---|
| [`ROLE-DESCRIPTION.md`](ROLE-DESCRIPTION.md) | What a member is asked to do, what authority they hold, and what the project owes them. |
| [`EXPRESSION-OF-INTEREST.md`](EXPRESSION-OF-INTEREST.md) | The public call. Plain language; an Easy Read pair is drafted at [`../easy-read/`](../easy-read/). |
| [`REMUNERATION.md`](REMUNERATION.md) | How members and supporters are paid, and the principles the rate must satisfy. The rate itself is a steward decision. |
| [`FIRST-MEETING.md`](FIRST-MEETING.md) | The agenda that constitutes LEDA, including its first act: deciding whether to ratify the framework it inherits. |

## The bootstrap problem

This is the substantive finding of drafting the pack, and it must be resolved before any of it goes out.

LEDA holds four consent gates. Three of them fire on the act of recruiting LEDA:

- **C1** — *every public claim about benefit to people with disability, before publication.* An expression of interest cannot avoid claiming that this work benefits people with disability. That is the claim it is built on.
- **C3** — *Easy Read and plain-language versions of the standard's public artefacts.* The drafting standard requires an Easy Read pair for public-facing artefacts. The recruitment pack is public-facing.
- **C4** — *changes to this framework itself.* The framework has no establishment clause. Adding one is a change to the framework.

So: **LEDA must consent to the documents that bring LEDA into existence.** The body cannot consent, because it does not exist; the steward must not publish over a withheld gate, and cannot obtain consent from a body with no members. Prime directive 4 admits no exceptions, "including *just this once*".
...
2. **An arms-length substitute for consent, not a waiver of it.** Before publication, the pack is reviewed and the Easy Read pair produced by a **disability-led organisation, paid at commercial rates**, engaged for that purpose and independent of the steward. This is weaker than LEDA consent and must be described as weaker. It is not a rubber stamp: the reviewer may require changes, and if they do, the pack does not go out until they are made.
...
1. Engage the disability-led reviewer and the Easy Read producer. Pay them.
2. Revise the pack to their direction. Record what changed.
3. Approach partner organisations with a funded role, not a favour.
4. Recruit 6–10 members against the composition targets in framework §4, plus supporters.
5. Hold the first meeting ([`FIRST-MEETING.md`](FIRST-MEETING.md)). LEDA ratifies, amends, or rejects.
6. Only then does gate C2 become reachable — and only then can any benchmark round be scored for publication.

Step 6 is why this is on the critical path. Nothing publishable clears without C2, and C2 cannot be given by a body that does not exist.
