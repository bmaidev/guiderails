...
3. **Parity boundary.** Anything not in this catalogue, not a conformance feature, and not a *derived divergence* listed in FIXTURE-SPEC.md §8 must be identical between builds. A derived divergence is one this catalogue forces: when a pattern removes a concept, everything keyed on that concept changes with it. The automated checks in `src/` assert exactly this boundary and fail CI on drift; the independent audit still signs.
