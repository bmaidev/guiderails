...
export const DECLARED_DIVERGENCES: Divergence[] = [
  ...
  { id: 'CF-authority-request', kind: 'conformance', description: 'An agent refused for lack of authority may request it from the principal via /api/authority-requests (write-only, creates no effect), and the principal grants or denies through a principal-only J4 review step (5.1.4). The baseline has no delegation model (B-07), so no authority to lack, request, or grant.' },
  { id: 'CF-availability', kind: 'conformance', description: 'A withdrawn machine surface answers 410 Gone with its successor, and a declared outage answers 503 with Retry-After, each legibly distinguishing permanent from temporary and naming an alternative channel (1.4.2). The baseline 404s an unknown path and has no outage semantics, so an agent cannot tell "gone" from "never existed".' },
