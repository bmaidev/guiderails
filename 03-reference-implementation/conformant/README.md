...
| OI-6 | **Closed.** The published step schema now describes the **request body** an agent must POST — the `values` envelope, the `confirmation` object with its token, and the optional `determinationId` — not merely the field names inside it (3.1.1). Exploratory smoke runs found two frontier models unable to complete J1 from the declared surface: the schema said `{declaration}` while the endpoint accepted `{values: {declaration}}`, and the resulting 422 named a field the agent had supplied. A wrong method now answers 405 with `Allow` rather than 404, a flat body is told about its envelope rather than about its fields, and a JSON body of literal `null` is a 400 rather than an unhandled 500. A test drives J1 end to end using only what the schema publishes. |
| OI-7 | **Closed.** Criterion 1.4.2: a withdrawn machine surface answers 410 Gone with its successor and a legible alternative channel; a declared outage answers 503 + Retry-After, distinguishing permanent from temporary — the sibling of the 405/Allow work. Withdrawn surfaces are declared in the service description (1.1.3), so the 410 is discoverable, not a surprise. Off-by-default outage registry; baseline untouched (CF-availability). |
